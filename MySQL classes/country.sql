...
SELECT id,sigla,ativo FROM estado;

SELECT estado.id,estado.sigla,estado.ativo FROM estado; 

SELECT
e.nome 'nome estado'
,e.sigla 'sigla estado'
,e.ativo 'se o estado esta ativo'
FROM estado e;									

SELECT  nome, data_cadastro FROM estado WHERE YEAR(data_cadastro) = 2020 AND ativo = 'N';

SELECT id,sigla FROM estado WHERE id>15;

SELECT id,nome,sigla,ativo,data_cadastro FROM estado WHERE id=7;

SELECT * FROM estado WHERE nome LIKE 'P%';

SELECT id,nome,sigla FROM estado WHERE sigla IN ('PR','SC','RS','AC' ,'RO' ,'AM' ,'RR' ,'PA' ,'AP' ,'TO');
